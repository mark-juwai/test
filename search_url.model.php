...
class search_url{   
    ...
    public function addOrChageSearch_key($search_key,$key,$value){
        switch($key){
            ...
            case 'gc':
            {
                ...
                if(!in_array($value,array('0','1','2','3','4','5','8'))){
                    TF_debug('没有这个gc');
                }
                break;
            }
            ...
            {   
                $search_key[$key]['key'] = $key;
                $search_key[$key]['field'] = 'search_order';
                $search_key[$key]['value'] = $value;
                $search_key[$key]['order'] = 15;
                if(!in_array($value,array('0','1','2','3','4','5','8'))){
                    TF_debug('没有这个gc');
                }
                break;
            ...
            {
                $search_key[$key]['key'] = $key;
                $search_key[$key]['field'] = 'index';
                $search_key[$key]['value'] = $value;
                $search_key[$key]['order'] = 16;
                if(preg_match('/^[0-9]{1,7}$/',$l['value'])===0){
                    TF_debug('没有这个i');
                }
                break;
            }
            ...
            {
                $search_key[$key]['key'] = $key;
                $search_key[$key]['field'] = 'page_item';
                $search_key[$key]['value'] = $value;
                $search_key[$key]['order'] = 17;
                if(!in_array($l['value'],array(10,20,50))){
                    TF_debug('没有这个it');
                }
                break;
            }
    ...
    public function removeSearch_key($search_key,$field){
        $new_search_key = array();
        if(is_array($field)){
            foreach($search_key as $k=>$l){
                if(!in_array($k,$field)){
                    $new_search_key[$k] = $l;
